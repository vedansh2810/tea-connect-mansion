...
class SimulatedDevice {
  ...
  placeOrderLoop() {
    ...
    placeOrder(this.userId, this.tableId).catch(() => {
      // Errors are logged inside placeOrder
    });
  }
...
async function runLoadTest() {
  ...
  console.log(`Number of simulated devices: ${TOTAL_VIRTUAL_USERS}`);
  console.log('Check Supabase dashboard for peak concurrent connections during the test.');
  console.log('If peak connections < 60, you have headroom.');
  console.log('If peak connections >= 60, you have reached/exceeded the limit.');
  console.log('='.repeat(60));
}
